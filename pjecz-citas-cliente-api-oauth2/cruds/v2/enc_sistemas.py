...
from ...models.enc_sistemas import EncSistema
from ...schemas.v2.enc_sistemas import EncSistemaIn
from ...settings import POLL_SYSTEM_URL, SALT
...
def get_enc_sistema_url(db: Session, cit_cliente_id: int) -> Optional[str]:
    ...
    enc_sistema = (
        db.query(EncSistema)
        .filter(EncSistema.cit_cliente_id == cit_cliente_id)
        .filter(EncSistema.estado == "PENDIENTE")
        .first()
    )
