...
from lib.santander_web_pay_plus import encrypt_chain, RESPUESTA_EXITO, RESPUESTA_ERROR, RESPUESTA_DENEGADA
...
def main():
    ...
    xml = ""
    with open("tests/respuesta_wpp.xml", "r") as file:
        for line in file:
            if "$" in line:
                indx_1 = line.find("$")
                indx_2 = line.rfind("$")
    ...
    xml_encriptado = encrypt_chain(xml).decode()
